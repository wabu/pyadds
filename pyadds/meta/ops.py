...
class Op(namedtuple('Op', 'name op reflect n kind format')):
    def __new__(cls, name, n, format, kind='unknown', reflect=False):
        op = get_op(name)
        reflect = Reflect(name, n, format, kind, reflect=False) if reflect else None
...
def iter_ops(cls, reflect=True):
    ...
    for op in operators:
        ...
        if op.reflect:
            ref = op.reflect
            if reflect and hasattr(cls, op.defines) or hasattr(cls, ref.defines):
                yield ref
...
def autowraped_ops(cls, by=None, reflect=True):
    ...
    ops = {op.defines: wrapping(op) for op in iter_ops(cls, reflect=reflect)}
